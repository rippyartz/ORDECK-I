...
function tbRenderGrid(){
  ...
  for(let i=1;i<=16;i++){const d=document.createElement('div');d.className='tb-gb';d.textContent=i;ghd.appendChild(d)}
  ...
  tbProj.tracks.forEach((tr,ti)=>{
    ...
    pat[ti].forEach((v,ci)=>{
      const c=document.createElement('div');c.className='tb-cell'+(v?' on':'');
      c.onclick=()=>{
        pat[ti][ci]=pat[ti][ci]?0:1;c.classList.toggle('on');touchProj('tb');
        if(pat[ti][ci]){ensureAudio();playSound(actx,tr.sound,tbMaster,tr.fx,actx.currentTime,tr.vol,tbProj.bpm,tr._buffer)}
      };
      r.appendChild(c);
    });
    grs.appendChild(r);
...
function tbScheduleStep(step,time){
  ...
  setTimeout(()=>{if(tbPlaying&&tbProj._playPattern===tbEditPattern)tbHighlight(step)},delay);
}
...
function tbToggleRec(){
  ...
  if(tbRec){
    if(!tbPlaying)tbTogglePlay();
    tbRecChunks=[];
    tbRecorder=new MediaRecorder(tbRecDest.stream);
    ...
    tbRecorder.onstop=()=>{
      ...
      const fname=(tbProj.name||'beat').replace(/[^a-z0-9_-]+/gi,'_')+'-'+Date.now()+'.webm';
      const row=document.createElement('a');row.href=url;row.download=fname;row.className='tb-recitem';
      row.innerHTML=`<i class="ti ti-download" aria-hidden="true"></i> ${fname}`;
      list.prepend(row);showToast('Recording ready to download');
    };
    tbRecorder.start();showToast('Recording...');
  } else {
    if(tbRecorder&&tbRecorder.state!=='inactive')tbRecorder.stop();
  }
  if(btn){btn.classList.toggle('on',tbRec);btn.style.borderColor=tbRec?'#f87171':'rgba(248,113,113,.28)';btn.style.color=tbRec?'#f87171':'rgba(248,113,113,.45)';btn.textContent=tbRec?'■ Rec':'Rec'}
}
